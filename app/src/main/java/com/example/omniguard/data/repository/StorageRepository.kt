package com.example.omniguard.data.repository
...
class StorageRepository @Inject constructor(
    ...
    fun getStorageAnalysis(): Flow<StorageAnalysis> {
        if (_storageAnalysis.value.totalSpace == 0L) {
            refreshStorageStats()
        }
    ...
    private suspend fun getStorageCategoryBreakdown(
        ...
        // Check for permissions before querying MediaStore
        ...
        // Fallback for Downloads using path query if MediaStore collection is empty or not supported
        if (downloadsSize == 0L) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            try {
                ...
                val appStats = statsManager.queryStatsForUser(StorageManager.UUID_DEFAULT, android.os.Process.myUserHandle())
